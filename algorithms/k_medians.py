...
def _get_dists(data, centers):
    ...
    for p in range(len(data)):
        dists[p] = [manhattan_dist(data[p], center) for center in centers]
    return dists
...
def _recompute_centers(data, k, labels):
    ...
    for i in range(k):
        ...
        centers[i] = [statistics.median(coord_group) for coord_group in coord_groups]
    return centers

def _no_shift(centers, new_centers, tol):
    shifts = [manhattan_dist(centers[i], new_centers[i]) for i in range(len(centers))]
    no_shift = all([shift <= tol for shift in shifts])
    return no_shift

def k_medians(data, k, tol=0.0001, max_iter=100, seed=1):
    """
    Implementation of k-Medians Clustering
